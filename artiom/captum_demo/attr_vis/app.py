...
class AttributionVisualizer:
    def __init__(
        ...
        self._config = FilterConfig(
            prediction="all", classes=[], num_examples=4)
    ...
    def _serve_colab(self, blocking=False, debug=False, port=None):
        ...
        out = widgets.Output()
        with out:
            port = start_server(self, blocking=blocking,
                                debug=debug, _port=port)
        shell = """
            <div id="root"></div>
            <script>
            (function() {
    ...
    def _should_keep_prediction(
        ...
        if self._config.prediction == "all":
            ...
            raise Exception(
                f"Invalid prediction config: {self._config.prediction}")
    ...
    def _calculate_vis_output(
        ...
        for model_index, model in enumerate(models_used):
            ...
            if target is None:
                target = (
                    predicted_scores[0].index if len(
    ...
    def _get_outputs(self) -> List[Tuple[List[VisualizationOutput], SampleCache]]:
        ...
        for (
            ...
            output = self._calculate_vis_output(
                inputs, additional_forward_args, label)
